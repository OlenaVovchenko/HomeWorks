...
-- Посчитать кол/во городов в каждой стране из таблицы locations. Вывести count_city, country_id
...
-- Вывести название должности(job_title) и маскимальная зарплата этой должности.
select t1.job_title, t2.max_salary
from jobs t1
join (SELECT job_id, max(salary) as max_salary
from employees group by job_id) t2
...
from (select department_id, count(*) as emp_conut
    ...
    -- 3 Найти ИД департамента с максимальным кол-вом сотрудников
